...
export function CollectionStats({ collection, stats, basePath }: CollectionStatsProps) {
  ...
  const gamesPlaying = stats?.gamesPlaying ?? collection?.filter(i => i.status === 'playing').length ?? 0;
  ...
  const cards = [
    ...
    { value: gamesPlaying, label: 'Playing', to: basePath ? `${basePath}?status=playing` : null },
    { value: gamesCompleted, label: 'Complete', to: basePath ? `${basePath}?status=completed` : null },
  ];
